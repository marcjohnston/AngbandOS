...
namespace byMarc.Net2.Library.LanguageParsing
{
    ...
    public class Parser
    {
        ...
        public Parser(string text, ParseLanguage languageConfig)
        {
            ...
            string whiteSpaceCharacters = Conversions.ToString(Operators.AddObject(" " + Constants.vbTab, Interaction.IIf(languageConfig.IgnoreLineFeeds, Constants.vbCrLf, "")));
            do
            {
                // Skip white space.
                        ...
                        else
                        {
                            ...
                            else if (languageConfig.RecognizeSingleQuoteLiterals && s == "'" || languageConfig.RecognizeDoubleQuoteLiterals && s == "\"")
                            {
                                ...
                                do
                                {
                                    ...
                                    else if (Conversions.ToString(PeekChar(characterIndex)) == literalChar)
                                    {
                                        // This is the closing literal.
                                        done = true;
                                    }
                                    else if (Conversions.ToBoolean(!languageConfig.AllowLiteralToSpanMultipleLines && (object)((PeekString(characterIndex, 2) ?? "") == Constants.vbCrLf || Conversions.ToString(PeekChar(characterIndex)) == Constants.vbCr || Conversions.ToString(PeekChar(characterIndex)) == Constants.vbLf)))
                                    {
                                ...
                                // Return the literal.
                                _tokenList.Add(new LiteralToken(leadingWhiteSpace, s, startPos, Strings.Mid(s, 2, Strings.Len(s) - 2)));
                            }
                            else
                            {
                                _tokenList.Add(new CharSymbolToken(leadingWhiteSpace, s, startPos));
