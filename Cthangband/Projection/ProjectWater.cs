...
namespace Cthangband.Projection
{
    internal class ProjectWater : Projectile
    {
        ...
        protected override bool AffectPlayer(int who, int r, int y, int x, int dam, int aRad)
        {
            ...
            if (fuzzy)
            {
                SaveGame.Instance.MsgPrint("You are hit by something wet!");
            }
            if (!Player.HasSoundResistance)
            {
                Player.SetTimedStun(Player.TimedStun + Program.Rng.DieRoll(40));
            }
            if (!Player.HasConfusionResistance)
            {
                Player.SetTimedConfusion(Player.TimedConfusion + Program.Rng.DieRoll(5) + 5);
            }
