...
internal partial class Game : IGameSerialize
{
    ...
    public GameStateBag? Serialize(SaveGameState saveGameState)
    {
        return new DictionaryGameStateBag(
            ...
            (nameof(Quests), saveGameState.CreateDerivedGameStateBag(Quests, typeof(Quest))),
            ...
            (nameof(AllocKindTable), saveGameState.CreateDerivedGameStateBag(AllocKindTable, typeof(AllocationEntry))),
            (nameof(AllocRaceSize), saveGameState.CreateGameStateBag(AllocRaceSize)),
            (nameof(AllocRaceTable), saveGameState.CreateDerivedGameStateBag(AllocRaceTable, typeof(AllocationEntry))),
            (nameof(CameFrom), saveGameState.CreateGameStateBag(CameFrom)),
            (nameof(CurDungeon), saveGameState.CreateDerivedGameStateBag(CurDungeon, typeof(Dungeon))),
            (nameof(CurrentDepth), saveGameState.CreateGameStateBag(CurrentDepth)),
            (nameof(CurTown), saveGameState.CreateDerivedGameStateBag(CurTown, typeof(Town))),
            ...
            (nameof(RecallDungeon), saveGameState.CreateDerivedGameStateBag(RecallDungeon, typeof(Dungeon))),
            ...
            (nameof(SpellOrder), saveGameState.CreateDerivedGameStateBag(SpellOrder, typeof(Spell))),
            ...
            (nameof(TownWithHouse), saveGameState.CreateDerivedGameStateBag(TownWithHouse, typeof(Town))),
            ...
            (nameof(Wilderness), saveGameState.CreateDerivedGameStateBag(Wilderness, typeof(WildernessRegion))),
            (nameof(WordOfRecallDelay), saveGameState.CreateGameStateBag(WordOfRecallDelay)),
            (nameof(Grid), saveGameState.CreateDerivedGameStateBag(Grid, typeof(GridTile))),
            ...
            (nameof(_prevPrimaryRealm), saveGameState.CreateDerivedGameStateBag(_prevPrimaryRealm, typeof(Realm))),
            (nameof(_prevSecondaryRealm), saveGameState.CreateDerivedGameStateBag(_prevSecondaryRealm, typeof(Realm))),
            (nameof(_prevSex), saveGameState.CreateDerivedGameStateBag(_prevSex, typeof(Gender))),
            (nameof(Inventory), saveGameState.CreateDerivedGameStateBag(Inventory, typeof(Item))),
    ...
    private Game(GameConfiguration gameConfiguration, (GameReplayDetails Details, bool CloseAfterReplay)? gameReplay, DerivedObjectGameStateBag? gameStateBag)
    {
        ...
        else
        {
            ...
            MissileAttacksPerRound = restoreGameState.GetByKey(nameof(MissileAttacksPerRound)).GetInt();
            OldSpareSpellSlots = restoreGameState.GetByKey(nameof(OldSpareSpellSlots)).GetInt();
            ...
            CurWid = restoreGameState.GetByKey(nameof(CurWid)).GetInt();
            DangerFeeling = restoreGameState.GetByKey(nameof(DangerFeeling)).GetInt();
            ...
            PanelColMax = restoreGameState.GetByKey(nameof(PanelColMax)).GetInt();
    ...
    public Dictionary<string, object> GetExpressionProviders(params (string, object)[]? additionalProviders)
    {
        ...
        else
        {
            ...
            {
                expressionProviders.Add(key, provider);
            }
        }
    ...
    {
        GridTile tile = Grid[MapY.IntValue][MapX.IntValue];
                    ...
                    if (!rPtr.KillWall && !rPtr.PassWall)
                    {
                        ...
                        if (!rPtr.NeverMove)
                        {
                            for (i = 0; i < 8; i++)
                            {
                                ...
                                if (Grid[y][x].FeatureType.IsEarthquakeResistant)
                                {
                                    continue;
                                }
                                if (map[16 + y - cy][16 + x - cx])
    ...
    { 
        ...
        bool oldTrapsDetected = Grid[MapY.IntValue][MapX.IntValue].TrapsDetected;
    ...
    public void SummonItem(int dir, int maxWeight, bool requireLos)
    {
        ...
        if (Grid[MapY.IntValue][MapX.IntValue].Items.Count > 0)
        {
        ...
        Grid[MapY.IntValue][MapX.IntValue].Items.Add(item);
    ...
    public void ProcessAllMonsters()
    {
        ...
        for (int i = MonsterMax - 1; i >= 1; i--)
        {
            ...
            else if (Grid[MapY.IntValue][MapX.IntValue].ScentAge == Grid[monsterY][monsterX].ScentAge &&
    ...
    public bool SummonPossible(int targetY, int targetX)
    {
        ...
        for (y = targetY - 2; y <= targetY + 2; y++)
        {
            ...
            for (x = targetX - 2; x <= targetX + 2; x++)
            {
                ...
                if (Grid[y][x].FeatureType.IsElderSignSigil)
                {
                    continue;
                }
                // Can't summon onto a Yellow Sign
                if (Grid[y][x].FeatureType.IsYellowSignSigil)
                {
                    continue;
                }
    ...
    public void ResolvePaths()
    {
        for (int x = 1; x < CurWid - 1; x++)
        {
            for (int y = 1; y < CurHgt - 1; y++)
            {
                if (!Grid[y][x].FeatureType.IsPathBase)
                {
                    continue;
                }
                int map = 0;
                ...
                switch (map)
                {
                    ...
                    case 4:
                    case 5:
                        Grid[y][x].SetFeature(SingletonRepository.Get<Tile>(nameof(PathNSTile)));
                        break;

                    case 2:
                    case 8:
                    case 10:
                        Grid[y][x].SetFeature(SingletonRepository.Get<Tile>(nameof(PathEWTile)));
                        break;

                    default:
                        Grid[y][x].SetFeature(SingletonRepository.Get<Tile>(nameof(PathJunctionTile)));
                        break;
                }
            }
        }
    ...
    public void SummonNamedMonster(bool slp)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            if (PlaceMonsterAux(y, x, rPtr, slp, true, false))
            {
                break;
            }
        }
    ...
    public bool Projectable(int y1, int x1, int y2, int x2)
    {
        ...
        for (int dist = 0; dist <= Constants.MaxRange; dist++)
        {
            ...
            if (dist != 0 && !GridPassable(y, x) && !Grid[y][x].FeatureType.IsYellowSignSigil)
            {
                break;
            }
    ...
    public bool SummonSpecific(int y1, int x1, int lev, MonsterRaceFilter? monsterFilter, bool groupOk, bool pet)
    {
        ...
        if (!PlaceMonsterAux(y, x, monsterRace, false, groupOk, pet))
        {
            return false;
        }
        return true;
