...
internal class UpdateBonusesFlaggedAction : FlaggedAction
{
    ...
    protected override void Execute()
    {
        ...
        Game.SkillUseDevice = Game.Race.BaseDeviceBonus + Game.BaseCharacterClass.BaseDeviceBonus; // added to attributes
        ...
        Game.SkillMelee = Game.Race.BaseMeleeAttackBonus + Game.BaseCharacterClass.BaseMeleeAttackBonus; // this appears to be tohit
        ...
        foreach (WieldSlot inventorySlot in Game.SingletonRepository.Get<WieldSlot>().Where(_inventorySlot => _inventorySlot.IsEquipment))
        {
            foreach (int i in inventorySlot.InventorySlots)
            {
                ...
                if (oPtr != null)
                {
                    ...
                    attackBonus += oPtr.EffectivePropertySet.MeleeToHit;
                    damageBonus += oPtr.EffectivePropertySet.ToDamage;
                    if (oPtr.IsKnown())
                    {
                        displayedAttackBonus += oPtr.EffectivePropertySet.MeleeToHit;
                    }
                    if (oPtr.IsKnown())
                    {
