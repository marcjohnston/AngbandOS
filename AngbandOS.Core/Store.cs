...
internal class Store
{
    ...
    private void StoreCreate()
    {
        ...
        for (int tries = 0; tries < 4; tries++)
        {
            ...
            newItem.EnchantItem(level.Value, false, false, false, true);
            newItem.BecomeKnown();
        ...
        foreach (StoreCommand command in Game.SingletonRepository.Get<StoreCommand>())
        {
            ...
            if (command.KeyChar == c)
            {
                ...
                if (command.IsEnabled(StoreFactory) && command.ExecuteScript != null)
                {
                    ...
                    command.ExecuteScript.ExecuteStoreCommandScript(storeCommandEvent);
    ...
    public void EnterStore()
    {
        ...
        while (!_leaveStore)
        {
            ...
            Game.NoticeStuff();
            Game.HandleStuff();
            const int itemIndex = InventorySlotEnum.PackCount;
