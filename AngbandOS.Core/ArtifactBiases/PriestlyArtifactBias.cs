...
internal class PriestlyArtifactBias : ArtifactBias
{
    ...
    public override BaseActivation GetActivationPowerType()
    {
        ...
        {
            return Game.SingletonRepository.Get<BaseActivation>(nameof(Heal1000Every888Activation));
        }
        else if (Game.DieRoll(8) == 1)
        {
            return Game.SingletonRepository.Get<BaseActivation>(nameof(Heal700Every25Activation));
        }
        else if (Game.DieRoll(7) == 1)
        {
            return Game.SingletonRepository.Get<BaseActivation>(nameof(RestAllActivation));
        }
        else if (Game.DieRoll(6) == 1)
        {
            return Game.SingletonRepository.Get<BaseActivation>(nameof(RestLifeActivation));
        }
        else
        {
            return Game.SingletonRepository.Get<BaseActivation>(nameof(Heal4d8AndWoundsEvery3p1d3Activation));
        }
    }
}
