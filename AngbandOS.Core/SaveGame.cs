 ...
 //       Dungeons = Dungeon.NewDungeonList(this);
    ...
    private Item? MakeFixedArtifact()
    {
        foreach (FixedArtifact aPtr in SingletonRepository.FixedArtifacts)
        {
            ...
            if (aPtr.Level > Difficulty)
            {
                ...
                if (Rng.RandomLessThan(d) != 0)
                {
                    continue;
                }
            }
            if (Rng.RandomLessThan(aPtr.Rarity) != 0)
            {
                return null;
            }
    ...
    public GoldItem MakeGold(int? goldType = null)
    {
        if (goldType == null)
        {
            ...
            goldType = ((Rng.DieRoll(Level.ObjectLevel + 2) + 2) / 2) - 1;

            // A great find has some probability.
            if (Rng.RandomLessThan(OneInProbabilityGoldItemIsGreat) == 0)
            {
                goldType += Rng.DieRoll(Level.ObjectLevel + 1);
            }
        }
            ...
            switch (Rng.DieRoll(27))
            {
                ...
                case 20:
                    if (!Player.HasFreeAction || Rng.DieRoll(100) >= Player.SkillSavingThrow)
                    {
                        ...
                        {
                            Player.TimedParalysis.AddTimer(Rng.DieRoll(3));
                        }
                        else
                        {
                            Player.TimedParalysis.AddTimer(Rng.DieRoll(13));
                        }
                    }
                    break;
    ...
    public void MonsterDeath(int mIdx)
    {
        ...
        mPtr.Items.Clear();
        if (rPtr.Drop60 && Rng.RandomLessThan(100) < 60)
        {
            number++;
        }
        if (rPtr.Drop90 && Rng.RandomLessThan(100) < 90)
        {
            number++;
        }
        if (rPtr.Drop_1D2)
    ...
    private void ProcessWorld()
    {
        ...
        if (Player.GameTime.IsMidnight)
        {
            ...
            {
                int town = Rng.RandomLessThan(SingletonRepository.Towns.Count);
        ...
        if (Player.Food < Constants.PyFoodWeak)
        {
            ...
            {
                if (Player.TimedParalysis.TurnsRemaining == 0 && Rng.RandomLessThan(100) < 10)
                {
                    MsgPrint("You faint from the lack of food.");
                    Disturb(true);
                    Player.TimedParalysis.AddTimer(1 + Rng.RandomLessThan(5));
                }
            }
        }
        if (Player.HasRegeneration)
    ...
    public void AcidDam(int dam, string kbStr)
    {
        ...
        if (!(Player.TimedAcidResistance.TurnsRemaining != 0 || Player.HasAcidResistance) && Rng.DieRoll(HurtChance) == 1)
        {
    ...
    public bool ApplyDisenchant()
    {
        ...
        string s;
        if ((oPtr.FixedArtifact != null || string.IsNullOrEmpty(oPtr.RandartName) == false) && Rng.RandomLessThan(100) < 71)
        {
            s = oPtr.Count != 1 ? "" : "s";
        ...
        if (oPtr.BonusArmorClass > 5 && Rng.RandomLessThan(100) < 20)
        {
            oPtr.BonusArmorClass--;
        }
    ...
    public bool BlessWeapon()
    {
        ...
        if (!(string.IsNullOrEmpty(oPtr.RandartName) == false || oPtr.FixedArtifact != null) || Rng.DieRoll(3) == 1)
        {
            string s = oPtr.Count > 1 ? "" : "s";
        ...
        else
        {
            ...
            if (oPtr.BonusArmorClass > 5 && Rng.RandomLessThan(100) < 33)
            {
                oPtr.BonusArmorClass--;
            }
            if (disHappened)
    ...
    public void ColdDam(int dam, string kbStr)
    {
        ...
        if (!(Player.TimedColdResistance.TurnsRemaining != 0 || Player.HasColdResistance) && Rng.DieRoll(HurtChance) == 1)
        {
    ...
    public void DestroyArea(int y1, int x1, int r)
    {
        ...
        if (flag)
        {
            ...
            {
                Player.TimedBlindness.AddTimer(10 + Rng.DieRoll(10));
            }
        }
                    ...
                    case 2:
                        {
                            ...
                            Player.TimedStun.AddTimer(Rng.DieRoll(50));
                            break;
                        }
                    case 3:
                        {
    ...
    public void ElecDam(int dam, string kbStr)
    {
        ...
        if (!(Player.TimedLightningResistance.TurnsRemaining != 0 || Player.HasLightningResistance) && Rng.DieRoll(HurtChance) == 1)
        {
    ...
    public void FireDam(int dam, string kbStr)
    {
        ...
        if (!(Player.TimedFireResistance.TurnsRemaining != 0 || Player.HasFireResistance) && Rng.DieRoll(HurtChance) == 1)
        {
            Player.TryDecreasingAbilityScore(Ability.Strength);
        }
        Player.TakeHit(dam, kbStr);
    ...
    public bool BashClosedDoor(int y, int x)
    {
        ...
        else
        {
            ...
            Player.TimedParalysis.AddTimer(2 + Rng.RandomLessThan(2));
        }
        return more;
    }
        ...
        while (num++ < Player.MeleeAttacksPerRound)
        {
            ...
            if (PlayerCheckHitOnMonster(chance, race.ArmourClass, monster.IsVisible))
            {
                ...
                if (meleeItem != null)
                {
                    ...
                    meleeItem.RefreshFlagBasedProperties();
                    chaosEffect = meleeItem.Characteristics.Chaotic && Rng.DieRoll(2) == 1;
                    if (meleeItem.Characteristics.Vampiric || (chaosEffect && Rng.DieRoll(5) < 3))
                    {
                ...
                if ((Player.BaseCharacterClass.ID == CharacterClass.Monk || Player.BaseCharacterClass.ID == CharacterClass.Mystic) && MartialArtistEmptyHands())
                {
                    ...
                    for (times = 0; times < (Player.Level < 7 ? 1 : Player.Level / 7); times++)
                    {
                        ...
                        do
                        {
                            martialArtsAttack = MaBlows[Rng.DieRoll(Constants.MaxMa) - 1];
                    ...
                    {
                        if (!race.Unique && Rng.DieRoll(Player.Level) > race.Level &&
                            monster.Speed > 60)
                        {
                ...
                else if (meleeItem != null)
                {
                    ...
                    totalDamage = Rng.DiceRoll(meleeItem.DamageDice, meleeItem.DamageDiceSides);
                    ...
                    if ((Player.HasQuakeWeapon && (totalDamage > 50 || Rng.DieRoll(7) == 1)) || (chaosEffect && Rng.DieRoll(250) == 1))
                    {
                        doQuake = true;
                        chaosEffect = false;
                    }
                    ...
                    bool vorpalCut = meleeItem.Characteristics.Vorpal && Rng.DieRoll(extraDamage1InChance) == 1;
                    ...
                    if (vorpalCut)
                    {
                        ...
                        {
                            totalDamage += stepK;
                        } while (Rng.DieRoll(meleeItem.FixedArtifact == null ? 4 : meleeItem.FixedArtifact.VorpalExtraAttacks1InChance) == 1);
                    }
    ...
    public void DoActivate()
    {
        ...
        if (chance < Constants.UseDevice && Rng.RandomLessThan(Constants.UseDevice - chance + 1) == 0)
        {
            chance = Constants.UseDevice;
        }

        // If we fail our use item roll just tell us and quit
        if (chance < Constants.UseDevice || Rng.DieRoll(chance) < Constants.UseDevice)
        {
        ...
        {
            TeleportPlayer(100);
            item.RechargeTimeLeft = 50 + Rng.DieRoll(50);
            return;
        }
    ...
    public void DoCmdThrow(int damageMultiplier)
    {
        ...
        int damage = Rng.DiceRoll(missile.DamageDice, missile.DamageDiceSides) + missile.BonusDamage;
        ...
        {
            if (hitBody || !Level.GridPassable(newY, newX) || Rng.DieRoll(100) < chanceToBreak)
            {
    ...
    public void DoGoUp()
    {
        ...
        else
        {
            ...
            int j = Rng.DieRoll(OneInChanceUpStairsReturnsToTownLevel);
            if (j > CurrentDepth)
            {
                j = 1;
            }
    ...
    public void DoCmdFire()
    {
        ...
        int shotDamage = Rng.DiceRoll(individualAmmunition.DamageDice, individualAmmunition.DamageDiceSides) + individualAmmunition.BonusDamage + missileWeapon.BonusDamage;
    ...
    public void DoCast()
    {
        if (Player.HasAntiMagic)
        {
            string whichMagicType = Player.BaseCharacterClass.SpellCastingType.MagicType;
        ...
        {
            Player.BaseCharacterClass.SpellCastingType.Cast();
        }
    }

    public void DoCmdCast()
    {
        string prayer = Player.BaseCharacterClass.SpellCastingType.SpellNoun;
        if (!Player.CanCastSpells)
        {
        ...
        else
        {
            ...
            Player.TimedParalysis.AddTimer(Rng.DieRoll((5 * oops) + 1));
            if (Rng.RandomLessThan(100) < 50)
            {
                bool perm = Rng.RandomLessThan(100) < 25;
                MsgPrint("You have damaged your health!");
                Player.DecreaseAbilityScore(Ability.Constitution, 15 + Rng.DieRoll(10), perm);
            }
        }
        RedrawManaFlaggedAction.Set();
    }
    ...
    public void DoCmdMentalism()
    {
        ...
        if (Rng.RandomLessThan(100) < chance)
        {
            ...
            if (Rng.DieRoll(100) < chance / 2)
            {
                int i = Rng.DieRoll(100);
                if (i < 5)
                {
                ...
                else if (i < 15)
                {
                    ...
                    Player.TimedHallucinations.AddTimer(5 + Rng.DieRoll(10));
                }
                else if (i < 45)
                {
                    MsgPrint("Your brain is addled!");
                    Player.TimedConfusion.AddTimer(Rng.DieRoll(8));
                }
                else if (i < 90)
                {
                    Player.TimedStun.AddTimer(Rng.DieRoll(8));
                }
                else
                {
        ...
        else
        {
            ...
            if (Rng.RandomLessThan(100) < 50)
            {
                ...
                Player.DecreaseAbilityScore(Ability.Wisdom, 15 + Rng.DieRoll(10), perm);
            }
        }
        RedrawManaFlaggedAction.Set();
    }
            ...
            case "SpikedPit":
                {
                    ...
                    else
                    {
                        ...
                        if (Rng.RandomLessThan(100) < 50)
                        {
                            ...
                            name = "a spiked pit";
                            damage *= 2;
                            Player.TimedBleeding.AddTimer(Rng.DieRoll(damage));
                        }
                        Player.TakeHit(damage, name);
                    }
                    break;
            ...
            case "PoisonPit":
                {
                    ...
                    else
                    {
                        ...
                        if (Rng.RandomLessThan(100) < 50)
                        {
                            ...
                            else if (Rng.DieRoll(10) <= Player.Religion.GetNamedDeity(GodName.Hagarg_Ryonis).AdjustedFavour)
                            {
                            ...
                            {
                                damage *= 2;
                                Player.TimedPoison.AddTimer(Rng.DieRoll(damage));
                            }
                        }
                        Player.TakeHit(damage, name);
                    }
            ...
            case "SlowDart":
                {
                    ...
                    if (TrapCheckHitOnPlayer(125))
                    {
                        ...
                        Player.TakeHit(damage, name);
                        Player.TimedSlow.AddTimer(Rng.RandomLessThan(20) + 20);
                    }
                    else
                    {
            ...
            case "BlindGas":
                {
                    ...
                    if (!Player.HasBlindnessResistance)
                    {
                        Player.TimedBlindness.AddTimer(Rng.RandomLessThan(50) + 25);
                    }
                    break;
                }
            case "ConfuseGas":
                {
                    ...
                    if (!Player.HasConfusionResistance)
                    {
                        Player.TimedConfusion.AddTimer(Rng.RandomLessThan(20) + 10);
                    }
                    break;
                }
            case "PoisonGas":
                {
                    ...
                    if (!Player.HasPoisonResistance && Player.TimedPoisonResistance.TurnsRemaining == 0)
                    {
                        ...
                        if (Rng.DieRoll(10) <= Player.Religion.GetNamedDeity(GodName.Hagarg_Ryonis).AdjustedFavour)
                        {
                            MsgPrint("Hagarg Ryonis's favour protects you!");
                        }
                        else
                        {
                            Player.TimedPoison.AddTimer(Rng.RandomLessThan(20) + 10);
                        }
                    }
                    break;
                }
            case "SleepGas":
                {
                    ...
                    if (!Player.HasFreeAction)
                    {
                        Player.TimedParalysis.AddTimer(Rng.RandomLessThan(10) + 5);
                    }
                    break;
                }
        }
    ...
    public void GetAhw()
    {
        Player.Age = Player.Race.BaseAge + Rng.DieRoll(Player.Race.AgeRange);
        bool startAtDusk = Player.Race.RestsTillDuskInsteadOfDawn;
        Player.GameTime = new GameTime(this, Rng.DieRoll(365), startAtDusk);

        if (Player.Gender.Index == Constants.SexMale)
        {
            Player.Height = Rng.RandomNormal(Player.Race.MaleBaseHeight, Player.Race.MaleHeightRange);
            Player.Weight = Rng.RandomNormal(Player.Race.MaleBaseWeight, Player.Race.MaleWeightRange);
        }
        else if (Player.Gender.Index == Constants.SexFemale)
        {
            Player.Height = Rng.RandomNormal(Player.Race.FemaleBaseHeight, Player.Race.FemaleHeightRange);
            Player.Weight = Rng.RandomNormal(Player.Race.FemaleBaseWeight, Player.Race.FemaleWeightRange);
        }
        else
        {
            if (Rng.DieRoll(2) == 1)
    ...
    {
        int gold = (Player.SocialClass * 6) + Rng.DieRoll(100) + 300;
        for (int i = 0; i < 6; i++)
        {
    ...
    public void GenerateNewLevel()
    {
        ...
        for (int generateAttemptNumber = 0; ; generateAttemptNumber++)
        {
            ...
            else
            {
                ...
                {
                    if (Rng.DieRoll(_smallLevel) == 1)
                    {
                        int tester1 = Rng.DieRoll(Level.MaxHgt / Constants.ScreenHgt);
                        int tester2 = Rng.DieRoll(Level.MaxWid / Constants.ScreenWid);
    ...
    private void AllocObject(int set, int typ, int num)
    {
        ...
        for (int k = 0; k < num; k++)
        {
            ...
            {
                dummy++;
                y = Rng.RandomLessThan(Level.CurHgt);
                x = Rng.RandomLessThan(Level.CurWid);
                if (!Level.GridOpenNoItemOrCreature(y, x))
                {
                    continue;
                }
    ...
    private void AllocStairs(string feat, int num, int walls)
    {
        for (int i = 0; i < num; i++)
        {
            for (bool flag = false; !flag;)
            {
                ...
                {
                    int y = Rng.RandomLessThan(Level.CurHgt);
                    int x = Rng.RandomLessThan(Level.CurWid);
                    if (!Level.GridOpenNoItemOrCreature(y, x))
                    {
                        continue;
                    }
    ...
    private void BuildField(int yy, int xx)
    {
        int y0 = (yy * 9) + 8;
        int x0 = (xx * 15) + 10;
        int y1 = y0 - Rng.DieRoll(2) - 1;
        int y2 = y0 + Rng.DieRoll(2) + 1;
        int x1 = x0 - Rng.DieRoll(3) - 2;
        int x2 = x0 + Rng.DieRoll(3) + 2;
        ...
        if (Rng.DieRoll(5) == 4)
        {
            int x = Rng.RandomBetween(x1, x2);
            int y = Rng.RandomBetween(y1, y2);
    ...
    private void BuildGraveyard(int yy, int xx)
    {
        ...
        int x1 = x0 - Rng.DieRoll(3) - 2;
        int x2 = x0 + Rng.DieRoll(3) + 2;
        for (int i = 0; i < Rng.RandomBetween(10, 20); i++)
        {
            int x = (Rng.RandomBetween(x1, x2) / 2 * 2) + 1;
            int y = (Rng.RandomBetween(y1, y2) / 2 * 2) + 1;
    ...
    {
        int dummy = 0;
        int y = Rng.RandomSpread(Level.CurHgt / 2, 10);
        int x = Rng.RandomSpread(Level.CurWid / 2, 15);
        int dir = Level.OrderedDirection[Rng.RandomLessThan(8)];
        while (dummy < SafeMaxAttempts)
        {
            dummy++;
    ...
    private void DestroyLevel()
    {
        ...
        {
            int x1 = Rng.RandomBetween(5, Level.CurWid - 1 - 5);
            int y1 = Rng.RandomBetween(5, Level.CurHgt - 1 - 5);
            int y;
    ...
    private void MakeDungeonEntrance(int left, int top, int width, int height, out int stairX, out int stairY)
    {
        ...
        {
            dummy++;
            y = Rng.RandomBetween(top, top + height);
            x = Rng.RandomBetween(left, left + width);
            if (Level.GridOpenNoItemOrCreature(y, x))
            {
                break;
            }
    ...
    private void BuildStores()
    {
        ...
        for (int i = 0; i < CurTown.Stores.Length; i++)
        {
            ...
            do
            {
                x = Rng.RandomBetween(0, 3);
                y = Rng.RandomBetween(0, 3);
    ...
    {
        GridTile cPtr;
        for (int n = 0; n < Rng.RandomBetween(1, 10) - 6; n++)
        {
            int x = Rng.RandomBetween(1, Level.CurWid - 2);
            int y = Rng.RandomBetween(1, Level.CurHgt - 2);
            cPtr = Level.Grid[y][x];
    ...
    {
        GridTile cPtr;
        for (int n = 0; n < Rng.RandomBetween(5, 10); n++)
        {
            int x = Rng.RandomBetween(1, Level.CurWid - 2);
            int y = Rng.RandomBetween(1, Level.CurHgt - 2);
            cPtr = Level.Grid[y][x];
    ...
    private GridCoordinate AddStairsDown()
    {
        ...
        {
            dummy++;
            y = Rng.RandomBetween(12, 29);
            x = Rng.RandomBetween(17, 46);
            if (Level.GridOpenNoItemOrCreature(y, x))
            {
                break;
            }
    ...
    private bool NewPlayerSpot()
    {
        ...
        while (maxAttempts-- != 0)
        {
            y = Rng.RandomBetween(1, Level.CurHgt - 2);
            x = Rng.RandomBetween(1, Level.CurWid - 2);
            if (!Level.GridOpenNoItemOrCreature(y, x))
            {
                continue;
            }
    ...
    private void PlaceRandomDoor(int y, int x)
    {
        ...
        {
            cPtr.SetFeature($"LockedDoor{Rng.DieRoll(7)}");
        }
        else
        {
    ...
    {
        Rng.UseFixed = true;
        Rng.FixedSeed = Wilderness[Player.WildernessY][Player.WildernessX].Seed;
        ...
        int rocks = Rng.RandomBetween(1, 10);
        for (int i = 0; i < rocks; i++)
        {
            x = Rng.DieRoll(Level.CurWid - 2);
            y = Rng.DieRoll(Level.CurHgt - 2);
    ...
    public bool GetDirectionNoAim(out int dp)
    {
        ...
        if (Player.TimedConfusion.TurnsRemaining != 0)
        {
            if (Rng.RandomLessThan(100) < 75)
            {
                dir = Level.OrderedDirection[Rng.RandomLessThan(8)];
            }
        }
    ...
    public void RandomizeInventory()
    {
        ...
        for (int i = 0; i < InventorySlot.PackCount; i++)
        {
            ...
            do
            {
                pos = Rng.RandomLessThan(InventorySlot.PackCount);
            }
    ...
    private int GetNumberMonster(int i)
    {
        ...
        num += Rng.RandomBetween(1, (Quests[i].Level / 3) + 5);
        return num;
    }
    ...
    private string AbilitySummary(int abilityIndex)
    {
        ...
        // Get the score
        AbilityScore ability = Player.AbilityScores[abilityIndex];
        ...
        if (Player.BaseCharacterClass.SpellStat == abilityIndex && abilityIndex != Ability.Strength)
        {
            ...
            if (Player.BaseCharacterClass.ID != CharacterClass.Mindcrafter && Player.BaseCharacterClass.ID != CharacterClass.Mystic && Player.BaseCharacterClass.ID != CharacterClass.Channeler)
            {
            ...
            if (Player.BaseCharacterClass.ID != CharacterClass.Channeler)
            {
    ...
    private void DisplayPlayerAbilityScoresWithEffects()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (Player.AbilityScores[i].Innate < Player.AbilityScores[i].InnateMax)
            {
                Screen.Print(Colour.Blue, Constants.StatNamesReduced[i], 14 + i, 1);
                int value = Player.AbilityScores[i].Adjusted;
                buf = value.StatToString();
                Screen.Print(Colour.Grey, buf, 14 + i, 6);
                buf = AbilitySummary(i);
                Screen.Print(Colour.Grey, buf, i + 14, 13);
            }
            else
            {
                Screen.Print(Colour.Blue, Constants.StatNames[i], 14 + i, 1);
                buf = Player.AbilityScores[i].Adjusted.StatToString();
                Screen.Print(Colour.Green, buf, 14 + i, 6);
                buf = AbilitySummary(i);
                Screen.Print(Colour.Green, buf, i + 14, 13);
            }
        }
    }
    ...
    private void ShowBonus(bool hasSustain, bool hasStat, int typeSpecificValue, int row, int col)
    {
        if (hasSustain)
        {
            ...
            Screen.Print(Colour.Green, "s", row, col);
        }
        else if (hasStat)
        {
            // Show stat
            if (typeSpecificValue <= -10)
            {
                ...
                Screen.Print(Colour.Red, "*", row, col);
            }
            else if (typeSpecificValue < 0)
            {
                // Display negative value
                Screen.Print(Colour.Red, (char)('0' - (char)typeSpecificValue), row, col);
            }
            else if (typeSpecificValue >= 10)
            {
                // Display max 
                Screen.Print(Colour.Green, "*", row, col);
            }
            else if (typeSpecificValue > 0)
            {
                // Display positive value
                Screen.Print(Colour.Green, (char)('0' + (char)typeSpecificValue), row, col);
            }
        }
        else
        {
            // Display neutral
            Screen.Print(Colour.Grey, ".", row, col);
        }
    }
    ...
    private void DisplayPlayerAbilityScoresWithModifiers()
    {
        const int statCol = 1;
        const int row = 22;
        Screen.Print(Colour.Purple, "Initial", row - 1, statCol + 5);
        Screen.Print(Colour.Brown, "Race Class Mods", row - 1, statCol + 13);
        Screen.Print(Colour.Green, "Actual", row - 1, statCol + 29);
        Screen.Print(Colour.Red, "Reduced", row - 1, statCol + 36);
        // Loop through the scores
        for (int i = 0; i < 6; i++)
        {
            // Reverse engineer our equipment bonuses from our score
            int equipmentBonuses = 0;
            if (Player.AbilityScores[i].InnateMax > 18 && Player.AbilityScores[i].AdjustedMax > 18)
            {
                equipmentBonuses = (Player.AbilityScores[i].AdjustedMax - Player.AbilityScores[i].InnateMax) / 10;
            }
            if (Player.AbilityScores[i].InnateMax <= 18 && Player.AbilityScores[i].AdjustedMax <= 18)
            {
                equipmentBonuses = Player.AbilityScores[i].AdjustedMax - Player.AbilityScores[i].InnateMax;
            }
            if (Player.AbilityScores[i].InnateMax <= 18 && Player.AbilityScores[i].AdjustedMax > 18)
            {
                equipmentBonuses = ((Player.AbilityScores[i].AdjustedMax - 18) / 10) - Player.AbilityScores[i].InnateMax + 18;
            }
            if (Player.AbilityScores[i].InnateMax > 18 && Player.AbilityScores[i].AdjustedMax <= 18)
            {
                equipmentBonuses = Player.AbilityScores[i].AdjustedMax - ((Player.AbilityScores[i].InnateMax - 18) / 10) - 19;
            }
            // Take out the bonuses we got for our our race and profession
            equipmentBonuses -= Player.Race.AbilityBonus[i];
            equipmentBonuses -= Player.BaseCharacterClass.AbilityBonus[i];
            // Print each of the scores and bonuses
            Screen.Print(Colour.Blue, Constants.StatNames[i], row + i, statCol);
            string buf = Player.AbilityScores[i].InnateMax.StatToString();
            Screen.Print(Colour.Purple, buf, row + i, statCol + 4);
            buf = Player.Race.AbilityBonus[i].ToString("+0;-0;+0").PadLeft(3);
            Screen.Print(Colour.Brown, buf, row + i, statCol + 13);
            buf = Player.BaseCharacterClass.AbilityBonus[i].ToString("+0;-0;+0").PadLeft(3);
            Screen.Print(Colour.Brown, buf, row + i, statCol + 19);
            buf = equipmentBonuses.ToString("+0;-0;+0").PadLeft(3);
            Screen.Print(Colour.Brown, buf, row + i, statCol + 24);
            buf = Player.AbilityScores[i].AdjustedMax.StatToString();
            Screen.Print(Colour.Green, buf, row + i, statCol + 27);
            if (Player.AbilityScores[i].Adjusted < Player.AbilityScores[i].AdjustedMax)
            {
                buf = Player.AbilityScores[i].Adjusted.StatToString();
                Screen.Print(Colour.Red, buf, row + i, statCol + 35);
            }
        }

        // Print the bonuses for each score and each item we have
        int col = statCol + 44;
        Screen.Print(Colour.Blue, "abcdefghijklm@", row - 1, col);
        Screen.Print(Colour.Blue, "Modifications", row + 6, col);
        foreach (BaseInventorySlot inventorySlot in SingletonRepository.InventorySlots.Where(_inventorySlot => _inventorySlot.IsEquipment))
        {
        ...
        ItemCharacteristics playerCharacteristics = Player.GetAbilitiesAsItemFlags();
        DisplayPlayerStatWithModification(Player.Dna.StrengthBonus, playerCharacteristics.Str, row + 0, col);
        DisplayPlayerStatWithModification(Player.Dna.IntelligenceBonus, playerCharacteristics.Int, row + 1, col);
        DisplayPlayerStatWithModification(Player.Dna.WisdomBonus, playerCharacteristics.Wis, row + 2, col);
        DisplayPlayerStatWithModification(Player.Dna.DexterityBonus, playerCharacteristics.Dex, row + 3, col);
        DisplayPlayerStatWithModification(Player.Dna.ConstitutionBonus, playerCharacteristics.Con, row + 4, col);
        DisplayPlayerStatWithModification(Player.Dna.CharismaBonus, playerCharacteristics.Cha, row + 5, col);
    }
    ...
    {
        int showTohit = Player.DisplayedAttackBonus;
        int showTodam = Player.DisplayedDamageBonus;
        MeleeWeaponInventorySlot meeleeWeaponInventorySlot = SingletonRepository.InventorySlots.Get<MeleeWeaponInventorySlot>();
        Item? item = GetInventoryItem(meeleeWeaponInventorySlot.WeightedRandom.Choose());
        ...
        PrintBonus("+ To AC     ", Player.DisplayedArmourClassBonus, 32, 1, Colour.Brown);
        PrintShortScore("  Base AC   ", Player.DisplayedBaseArmourClass, 33, 1, Colour.Brown);
        PrintShortScore("Level      ", Player.Level, 30, 28, Colour.Green);
        PrintLongScore("Experience ", Player.ExperiencePoints, 31, 28,
            Player.ExperiencePoints >= Player.MaxExperienceGained ? Colour.Green : Colour.Red);
        PrintLongScore("Max Exp    ", Player.MaxExperienceGained, 32, 28, Colour.Green);
        // If we're max level we don't have any experience to advance
        if (Player.Level >= Constants.PyMaxLevel)
        {
            Screen.Print(Colour.Blue, "Exp to Adv.", 33, 28);
            Screen.Print(Colour.Green, "    *****", 33, 28 + 11);
        }
        else
        {
            PrintLongScore("Exp to Adv.", (int)(Constants.PlayerExp[Player.Level - 1] * Player.ExperienceMultiplier / 100L), 33, 28,
                Colour.Green);
        }
        PrintLongScore("Exp Factor ", Player.ExperienceMultiplier, 34, 28, Colour.Green);
        PrintShortScore("Max Hit Points ", Player.MaxHealth, 30, 52, Colour.Green);
        if (Player.Health >= Player.MaxHealth)
        {
            PrintShortScore("Cur Hit Points ", Player.Health, 31, 52, Colour.Green);
        }
        else if (Player.Health > Player.MaxHealth * Constants.HitpointWarn / 10)
        {
            PrintShortScore("Cur Hit Points ", Player.Health, 31, 52, Colour.BrightYellow);
        }
        else
        {
            PrintShortScore("Cur Hit Points ", Player.Health, 31, 52, Colour.BrightRed);
        }
        PrintShortScore("Max SP (Mana)  ", Player.MaxMana, 32, 52, Colour.Green);
        if (Player.Mana >= Player.MaxMana)
        {
            PrintShortScore("Cur SP (Mana)  ", Player.Mana, 33, 52, Colour.Green);
        }
        else if (Player.Mana > Player.MaxMana * Constants.HitpointWarn / 10)
        {
            PrintShortScore("Cur SP (Mana)  ", Player.Mana, 33, 52, Colour.BrightYellow);
        }
        else
        {
            PrintShortScore("Cur SP (Mana)  ", Player.Mana, 33, 52, Colour.BrightRed);
        }
        PrintLongScore("Gold           ", Player.Gold, 34, 52, Colour.Green);
    }
    ...
    private void DisplayPlayerHistory()
    {
        for (int i = 0; i < 4; i++)
        {
            Screen.Print(Colour.Brown, Player.History[i], i + 9, 10);
        }
    }
    ...
    private void DisplayPlayerSkills()
    {
        ...
        Item? meeleeItem = GetInventoryItem(index);

        int dambonus = Player.DisplayedDamageBonus;
        // Only include weapon damage if the player knows what it is
        int damdice = 0;
        int damsides = 0;
        int fighting = Player.SkillMelee + (Player.AttackBonus * Constants.BthPlusAdj);
        if (meeleeItem != null)
        {
        ...
        RangedWeaponInventorySlot rangedWeaponInventorySlot = SingletonRepository.InventorySlots.Get<RangedWeaponInventorySlot>();
        Item? rangedItem = GetInventoryItem(rangedWeaponInventorySlot.WeightedRandom.Choose());
        int shooting = Player.SkillRanged + (Player.AttackBonus * Constants.BthPlusAdj);
        if (rangedItem != null)
        {
            shooting += rangedItem.BonusToHit * Constants.BthPlusAdj;
        }

        int attacksPerRound = Player.MeleeAttacksPerRound;
        int disarmTraps = Player.SkillDisarmTraps;
        int useDevice = Player.SkillUseDevice;
        int savingThrow = Player.SkillSavingThrow;
        int stealth = Player.SkillStealth;
        int searching = Player.SkillSearching;
        int searchFrequency = Player.SkillSearchFrequency;
        Screen.Print(Colour.Blue, "Fighting    :", 36, 1);
        PrintCategorisedNumber(fighting, 12, 36, 15);
        Screen.Print(Colour.Blue, "Shooting    :", 37, 1);
        PrintCategorisedNumber(shooting, 12, 37, 15);
        Screen.Print(Colour.Blue, "Saving Throw:", 38, 1);
        PrintCategorisedNumber(savingThrow, 6, 38, 15);
        Screen.Print(Colour.Blue, "Stealth     :", 39, 1);
        PrintCategorisedNumber(stealth, 1, 39, 15);
        Screen.Print(Colour.Blue, "Perception  :", 36, 28);
        PrintCategorisedNumber(searchFrequency, 6, 36, 42);
        Screen.Print(Colour.Blue, "Searching   :", 37, 28);
        PrintCategorisedNumber(searching, 6, 37, 42);
        Screen.Print(Colour.Blue, "Disarming   :", 38, 28);
        PrintCategorisedNumber(disarmTraps, 8, 38, 42);
        Screen.Print(Colour.Blue, "Magic Device:", 39, 28);
        PrintCategorisedNumber(useDevice, 6, 39, 42);
        Screen.Print(Colour.Blue, "Blows/Action:", 36, 55);
        Screen.Print(Colour.Green, $"{Player.MeleeAttacksPerRound}", 36, 69);
        Screen.Print(Colour.Blue, "Tot.Dmg./Act:", 37, 55);
        ...
        Screen.Print(Colour.Green, buf, 37, 69);
        Screen.Print(Colour.Blue, "Shots/Action:", 38, 55);
        Screen.Print(Colour.Green, $"{Player.MissileAttacksPerRound}", 38, 69);
        Screen.Print(Colour.Blue, "Infra-Vision:", 39, 55);
        Screen.Print(Colour.Green, $"{Player.InfravisionRange * 10} feet", 39, 69);
    }
    ...
    private void DisplayPlayerTop()
    {
        ...
        Screen.Print(Colour.Brown, Player.Name, 2, 15);
        Screen.Print(Colour.Brown, Player.Gender.Title, 3, 15);
        Screen.Print(Colour.Brown, Player.Race.Title, 4, 15);
        Screen.Print(Colour.Brown, Player.BaseCharacterClass.ClassSubName(Player.PrimaryRealm), 5, 15);
        // Only print realms if we have them
        if (Player.PrimaryRealm != null)
        {
            string realmBuff = RealmNames(Player.PrimaryRealm, Player.SecondaryRealm);
            Screen.Print(Colour.Brown, realmBuff, 6, 15);
        }
        // Fanatics and Cultists get a patron
        if (Player.BaseCharacterClass.ID == CharacterClass.Fanatic || Player.BaseCharacterClass.ID == CharacterClass.Cultist)
        {
            Screen.Print(Colour.Blue, "Patron      :", 7, 1);
            Screen.Print(Colour.Brown, Player.GooPatron.LongName, 7, 15);
        }
        // Priests get a deity
        if (Player.Religion.Deity != GodName.None)
        {
            Screen.Print(Colour.Blue, "Deity       :", 7, 1);
            Screen.Print(Colour.Brown, Player.Religion.GetPatronDeity().LongName, 7, 15);
        }
        Screen.Print(Colour.Blue, "Birthday", 2, 32);
        string dateBuff = Player.GameTime.BirthdayText.PadLeft(8);
        Screen.Print(Colour.Brown, dateBuff, 2, 46);
        PrintShortScore("Age          ", Player.Age, 3, 32, Colour.Brown);
        PrintShortScore("Height       ", Player.Height, 4, 32, Colour.Brown);
        PrintShortScore("Weight       ", Player.Weight, 5, 32, Colour.Brown);
        PrintShortScore("Social Class ", Player.SocialClass, 6, 32, Colour.Brown);
        int i;
        ...
        for (i = 0; i < 6; i++)
        {
            ...
            if (Player.AbilityScores[i].Innate < Player.AbilityScores[i].InnateMax)
            {
                Screen.Print(Colour.Blue, Constants.StatNamesReduced[i], 2 + i, 61);
                int value = Player.AbilityScores[i].Adjusted;
                buf = value.StatToString();
                Screen.Print(Colour.Red, buf, 2 + i, 66);
                value = Player.AbilityScores[i].AdjustedMax;
                buf = value.StatToString();
                Screen.Print(Colour.Green, buf, 2 + i, 73);
            }
            else
            {
                Screen.Print(Colour.Blue, Constants.StatNames[i], 2 + i, 61);
                buf = Player.AbilityScores[i].Adjusted.StatToString();
                Screen.Print(Colour.Green, buf, 2 + i, 66);
            }
        }
    }
    ...
    private void PrintBonus(string header, int num, int row, int col, Colour colour)
    {
        ...
        Screen.Print(Colour.Blue, header, row, col);
        Screen.Print(Colour.Blue, "   ", row, col + len);
        string outVal = num.ToString("+0;-0;0").PadLeft(6);
        Screen.Print(colour, outVal, row, col + len + 3);
    }
    ...
    private void PrintCategorisedNumber(int score, int divider, int screenRow, int screenCol)
    {
        ...
        Screen.Print(colour, text, screenRow, screenCol);
    }
    ...
    private void PrintLongScore(string title, int number, int row, int col, Colour colour)
    {
        ...
        Screen.Print(Colour.Blue, title, row, col);
        string outVal = number.ToString().PadLeft(9);
        Screen.Print(colour, outVal, row, col + len);
    }
    ...
    public void ShowMenu()
    {
        ...
        while (true && !Shutdown)
        {
            UpdateScreen();
            Screen.Clear();
            Screen.Print(Colour.Blue, "Journal", 0, 1);
            Screen.Print(Colour.Blue, "=======", 1, 1);
            Screen.Print(Colour.Blue, "(a) Abilities", 3, 0);
            Screen.Print(Colour.Blue, "(d) Deities", 4, 0);
            Screen.Print(Colour.Blue, "(k) Kill Count", 5, 0);
            Screen.Print(Colour.Blue, "(m) Mutations", 6, 0);
            Screen.Print(Colour.Blue, "(p) Pets", 7, 0);
            Screen.Print(Colour.Blue, "(q) Quests", 8, 0);
            Screen.Print(Colour.Blue, "(r) Word of Recall", 9, 0);
            Screen.Print(Colour.Blue, "(s) Monsters Seen", 10, 0);
            Screen.Print(Colour.Blue, "(u) Uniques", 11, 0);
            Screen.Print(Colour.Blue, "(w) Worthless Items", 12, 0);
            Screen.Print(Colour.Orange, "[Select a journal section, or Escape to finish.]", 43, 1);
            char k = Inkey();
            if (k == '\x1b')
            {
                break;
    ...
    private void DisplayStat(string title, int row, int col, Func<IItemCharacteristics, bool> getStat)
    {
        ...
        Screen.Print(baseColour, title, row, col);
        Screen.Print(baseColour, ':', row, col + 10); // Right aligned
        ...
        foreach (BaseInventorySlot inventorySlot in SingletonRepository.InventorySlots.Where(_inventorySlot => _inventorySlot.IsEquipment).OrderBy(_inventorySlot => _inventorySlot.SortOrder))
        {
            foreach (int i in inventorySlot.InventorySlots)
            {
                ...
                if (thisHasStat)
                {
                    Screen.Print(baseColour, "+", row, col + 10 + index + 1);
                }
                else
                {
                    Screen.Print(Colour.Grey, ".", row, col + 10 + index + 1);
                }
                index++;
            }
        }
        ...
        {
            Screen.Print(baseColour, "+", row, col + 10 + 26); // col + 10 + InventorySlot.Total - InventorySlot.MeleeWeapon + 1);
        }
        else
        {
            Screen.Print(Colour.Grey, ".", row, col + 10 + 26); // col + 10 + InventorySlot.Total - InventorySlot.MeleeWeapon + 1);
        }
    ...
    private void DisplayMonster(int rIdx, int num, int of)
    {
        ...
        {
            if (Constants.SymbolIdentification[i][0] == SingletonRepository.MonsterRaces[rIdx].Character)
            {
                string name = Constants.SymbolIdentification[i].Substring(2);
                string buf = $"Monster Type: {name} ({num + 1} of {of})";
                Screen.Print(Colour.Blue, buf, 3, 0);
                break;
            }
        }
        Screen.Goto(5, 0);
        ...
        Screen.Goto(6, 0);
        SingletonRepository.MonsterRaces[rIdx].Knowledge.DisplayBody(Colour.Brown);
    }
    ...
    private void DisplayMonsterHeader(int rIdx)
    {
        ...
        if (!rPtr.Unique)
        {
            Screen.Print(Colour.Brown, "The ");
        }
        Screen.Print(Colour.Brown, $"{rPtr.Name} ('");
        Screen.Print(a1, c1.ToString());
        Screen.Print(Colour.Brown, "')");
    }
    ...
    private void JournalAbilities()
    {
        ...
        DisplayPlayerEquippy(0, 0 + 11);
        Screen.Print(Colour.Blue, "abcdefghijklm@", 1, 0 + 11);
        ...
        DisplayPlayerEquippy(0, 26 + 11);
        Screen.Print(Colour.Blue, "abcdefghijklm@", 1, 26 + 11);
        ...
        DisplayPlayerEquippy(0, 52 + 11);
        Screen.Print(Colour.Blue, "abcdefghijklm@", 1, 52 + 11);
        ...
        DisplayPlayerEquippy(20, 0 + 11);
        Screen.Print(Colour.Blue, "abcdefghijklm@", 21, 0 + 11);
        ...
        DisplayPlayerEquippy(20, 26 + 11);
        Screen.Print(Colour.Blue, "abcdefghijklm@", 21, 26 + 11);
        ...
        DisplayPlayerEquippy(20, 52 + 11);
        Screen.Print(Colour.Blue, "abcdefghijklm@", 21, 52 + 11);
        ...
        Screen.Print(Colour.Orange, "[Press any key to finish.]", 43, 1);
        Inkey();
    }
    ...
    {
        Screen.Clear();
        Screen.Print(Colour.Blue, "Standings with Deities", 0, 1);
        Screen.Print(Colour.Blue, "======================", 1, 1);
        int row = 3;
        God patron = null;
        foreach (var deity in Player.Religion.GetAllDeities())
        {
            ...
            text += ".";
            Screen.Print(Colour.Blue, text, row, 1);
            row++;
        }
        if (patron != null)
        {
            Screen.Print(Colour.Blue, $"You are a follower of {patron.LongName}.", 12, 1);
            Screen.Print(Colour.Blue, $"Over time, your standing with {patron.ShortName} will revert to approval.", 13, 1);
            Screen.Print(Colour.Blue, $"Your standing with other deities will revert to annoyance.", 14, 1);
        }
        else
        {
            Screen.Print(Colour.Blue, "Over time, your standing with all deities will revert back to indifference.", 12, 1);
        }
        Screen.Print(Colour.Orange, "[Press any key to finish.]", 43, 1);
        Inkey();
    }
    ...
    {
        string[] names = new string[SingletonRepository.MonsterRaces.Count];
        int[] counts = new int[SingletonRepository.MonsterRaces.Count];
        bool[] unique = new bool[SingletonRepository.MonsterRaces.Count];
        int maxCount = 0;
        int total = 0;
        for (int i = 0; i < SingletonRepository.MonsterRaces.Count - 1; i++)
        {
            MonsterRace monster = SingletonRepository.MonsterRaces[i];
            if (monster.Unique)
            {
        ...
        while (true && !Shutdown)
        {
            string buf;
            Screen.Clear();
            Screen.Print(Colour.Blue, "Kill Count", 0, 1);
            Screen.Print(Colour.Blue, "==========", 1, 1);
            if (maxCount == 0)
            {
                Screen.Print(Colour.Blue, "You haven't killed anything yet!", 3, 0);
            }
            for (int i = first; i < first + 38; i++)
            {
                if (i < maxCount)
                {
                    ...
                    Screen.Print(Colour.Blue, buf, i - first + 3, 0);
                }
            }
            buf = $"Total Kills: {total}";
            Screen.Print(Colour.Blue, buf, 41, 0);
            Screen.Print(Colour.Orange, "[Use up and down to navigate list, and Escape to finish.]", 43, 1);
            int c = Inkey();
            if (c == '\x1b')
            {
                break;
    ...
    {
        int[] seen = new int[SingletonRepository.MonsterRaces.Count];
        int[] filtered = new int[SingletonRepository.MonsterRaces.Count];
        int maxSeen = 0;
        ...
        {
            if (SingletonRepository.MonsterRaces[i].Knowledge.RSights != 0 || Player.IsWizard)
            {
                seen[maxSeen] = i;
                maxSeen++;
                char symbol = SingletonRepository.MonsterRaces[i].Character;
        ...
        {
            Screen.Clear();
            Screen.Print(Colour.Blue, "Monsters Seen", 0, 1);
            Screen.Print(Colour.Blue, "=============", 1, 1);
            Screen.Print(Colour.Blue, "You haven't seen any monsters yet!", 3, 0);
            Screen.Print(Colour.Orange, "[Press any key to finish]", 43, 1);
            Inkey();
            return;
        }
        ...
        while (true)
        {
            ...
            {
                if (SingletonRepository.MonsterRaces[seen[i]].Character == currentFilter)
                {
    ...
    {
        string[] features = Player.Dna.GetMutationList();
        int maxFeature = features.Length;
        int first = 0;
        while (true && !Shutdown)
        {
            Screen.Clear();
            Screen.Print(Colour.Blue, "Mutations", 0, 1);
            Screen.Print(Colour.Blue, "=========", 1, 1);
            if (maxFeature == 0)
            {
                Screen.Print(Colour.Blue, "You have no mutations.", 3, 0);
            }
            else
            {
                for (int i = first; i < first + 38; i++)
                {
                    if (i < maxFeature)
                    {
                        Screen.Print(Colour.Blue, features[i], i - first + 3, 0);
                    }
                }
            }
            Screen.Print(Colour.Orange, "[Use up and down to navigate list, and Escape to finish.]", 43, 1);
    ...
    private void JournalPets()
    {
        ...
        {
            Screen.Clear();
            Screen.Print(Colour.Blue, "Pets", 0, 1);
            Screen.Print(Colour.Blue, "====", 1, 1);
            if (pets == 0)
            {
                Screen.Print(Colour.Blue, "You have no pets.", 3, 0);
            }
            else
            {
                for (int i = first; i < first + 38; i++)
                {
                    if (i < pets)
                    {
                        Screen.Print(Colour.Blue, petNames[i], i - first + 3, 0);
                    }
                }
            }
            Screen.Print(Colour.Orange, "[Use up and down to navigate list, and Escape to finish.]", 43, 1);
    ...
    {
        Screen.Clear();
        Screen.Print(Colour.Blue, "Outstanding Quests", 0, 1);
        Screen.Print(Colour.Blue, "==================", 1, 1);
        int row = 3;
        for (int i = 0; i < DungeonCount; i++)
        {
            int firstQuest = SingletonRepository.Dungeons[i].FirstQuest();
            if (firstQuest != -1)
            {
                string line = Quests[firstQuest].Describe();
                Screen.Print(Colour.Blue, line, row, 0);
                row++;
            }
        }
        if (row == 3)
        {
            Screen.Print(Colour.Blue, "Congratulations! You have completed all the quests.", row, 0);
        }
        Screen.Print(Colour.Orange, "[Press any key to finish.]", 43, 1);
        Inkey();
    }
    ...
    {
        Screen.Clear();
        Screen.Print(Colour.Blue, "Word of Recall", 0, 1);
        Screen.Print(Colour.Blue, "==============", 1, 1);
        string recallTown = Player.TownWithHouse > -1 ? SingletonRepository.Towns[Player.TownWithHouse].Name : CurTown.Name;
        string recallDungeon = RecallDungeon.Name;
        int recallLev = RecallDungeon.RecallLevel;
        Screen.Print(Colour.Blue, $"Your Word of Recall position is level {recallLev} of {recallDungeon}.", 3, 0);
        Screen.Print(Colour.Blue, $"Your home town is {recallTown}.", 4, 0);
        if (Player.TownWithHouse > -1)
        {
            recallTown = "your house in " + SingletonRepository.Dungeons[Player.TownWithHouse].Shortname;
        }
        Screen.Print(Colour.Brown,
            CurrentDepth == 0
        ...
        Screen.Goto(8, 0);
        Screen.PrintWrap(Colour.Blue, description);
        Screen.Print(Colour.Orange, "[Press any key to finish.]", 43, 1);
        Inkey();
    }
    ...
    {
        string[] names = new string[SingletonRepository.MonsterRaces.Count];
        bool[] alive = new bool[SingletonRepository.MonsterRaces.Count];
        int maxCount = 0;
        for (int i = 0; i < SingletonRepository.MonsterRaces.Count - 1; i++)
        {
            MonsterRace monster = SingletonRepository.MonsterRaces[i];
            if (monster.Unique &&
                (monster.Knowledge.RSights > 0 || Player.IsWizard))
            {
                names[maxCount] = monster.Name;
                bool dead = monster.MaxNum == 0;
                alive[maxCount] = !dead;
        ...
        {
            Screen.Clear();
            Screen.Print(Colour.Blue, "Unique Foes", 0, 1);
            Screen.Print(Colour.Blue, "===========", 1, 1);
            if (maxCount == 0)
            {
                Screen.Print(Colour.Blue, "You know of no unique foes!", 3, 0);
            }
            for (int i = first; i < first + 38; i++)
            {
                if (i < maxCount)
    ...
    {
        Screen.Clear();
        Screen.Print(Colour.Blue, "Worthless Items", 0, 1);
        Screen.Print(Colour.Blue, "===============", 1, 1);
        Screen.Goto(3, 0);
        ...
        Screen.PrintWrap(Colour.Blue, text);
        ...
        {
            MenuDisplay(menu);
            Screen.Print(Colour.Orange, "[Up/Down = select item type, Left/Right = forward/back.]", 43, 1);
            while (true)
            {
                char c = Inkey();
                if (c == '8' && menu > 0)
    ...
    {
        Screen.Clear(9);
        Screen.Print(Colour.Orange, "=>", 25, 0);
        ...
        Screen.Print(descColour, desc, 25, 33);
    }
    ...
    {
        _menuLength = 0;
        for (int i = 0; i < SingletonRepository.ItemFactories.Count; i++)
        {
            ItemFactory kPtr = SingletonRepository.ItemFactories[i];
            if (kPtr.CategoryEnum == tval)
            {
        ...
        while (true && !Shutdown)
        {
            ...
            while (true)
            {
                ...
                if (c == '6')
                {
                    ItemFactory kPtr = SingletonRepository.ItemFactories[_menuIndices[menu]];
                    if (kPtr.HasQuality)
                    {
