    ...
    public override void UseRacialPower()
    {
        ...
        if (Game.CheckIfRacialPowerWorks(9, 9, AbilityEnum.Dexterity, 14))
        {
            if (Game.GetDirectionWithAim(out int direction))
            {
                ...
                {
                    Game.FireBolt(Game.SingletonRepository.Get<Projectile>(nameof(ProjectileNamesEnum.AcidProjectile)), direction, Game.ExperienceLevel.IntValue);
                }
                else
                {
                    Game.FireBall(Game.SingletonRepository.Get<Projectile>(nameof(ProjectileNamesEnum.AcidProjectile)), direction, Game.ExperienceLevel.IntValue, 2);
                }
            }
        }
    }
