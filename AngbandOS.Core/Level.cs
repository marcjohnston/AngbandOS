...
namespace AngbandOS.Core
{
    ...
    internal class Level
    {
        ...
        public Item? ChestCheck(int y, int x)
        {
            ...
            {
                Item? oPtr = SaveGame.GetLevelItem(objectItemIndex);
        ...
        public void CompactObjects(int size)
        {
            ...
            for (num = 0, cnt = 1; num < size; cnt++)
            {
                ...
                for (i = 1; i < OMax; i++)
                {
                    ...
                    if (curDis > 0 && Distance(SaveGame.Player.MapY, SaveGame.Player.MapX, y, x) < curDis)
                    {
                        continue;
                    }
                    int chance = 90;
        ...
        public void DeleteObject(int y, int x)
        {
            ...
            for (int thisOIdx = cPtr.ItemIndex; thisOIdx != 0; thisOIdx = nextOIdx)
            {
                ...
                nextOIdx = oPtr.NextInStack;
                SaveGame.SetLevelItem(thisOIdx, null);
                OCnt--;
            }
            cPtr.ItemIndex = 0;
            RedrawSingleLocation(y, x);
        ...
        public void DeleteObjectIdx(int oIdx)
        {
            ...
            SaveGame.SetLevelItem(oIdx, null);
            OCnt--;
        }
        ...
        public void DropNear(Item jPtr, int chance, int y, int x)
        {
            ...
            if (!done)
            {
                SaveGame.SetLevelItem(oIdx, jPtr.Clone());
                Item? oPtr = SaveGame.GetLevelItem(oIdx);
                if (oPtr == null)
                {
        ...
        public void PlaceGold(int y, int x)
        {
            ...
            if (oIdx != 0)
            {
                SaveGame.SetLevelItem(oIdx, qPtr.Clone());
                Item? oPtr = SaveGame.GetLevelItem(oIdx);
                if (oPtr == null)
                {
                    return;
