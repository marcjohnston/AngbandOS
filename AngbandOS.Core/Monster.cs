...
internal class Monster : IItemContainer
{
    ...
    public void ProcessMonster(uint noise)
    {
        ...
        for (int i = 0; potentialMoves[i] != 0; i++)
        {
            ...
            if (d == 5)
            {
                d = SaveGame.OrderedDirection[Program.Rng.RandomLessThan(8)];
            }
                ...
                if (effect == null || MonsterCheckHitOnPlayer(SaveGame, power, monsterLevel))
                {
                    ...
                    if (touched)
                    {
                        if (saveGame.HasFireShield && alive)
                        {
                            ...
                            {
                                saveGame.MsgPrint($"{monsterName} is suddenly very hot!");
                                if (saveGame.DamageMonster(GetMonsterIndex(), Program.Rng.DiceRoll(2, 6), out fear,
    ...
    private MonsterSpell ChooseSpellAgainstPlayer(SaveGame saveGame, MonsterSpellList spells) // int[] spells, int listSize)
    {
        ...
        if (saveGame.Distance(saveGame.MapY, saveGame.MapX, MapY, MapX) < 4 && attackSpells.Count > 0 && Program.Rng.RandomLessThan(100) < 75)
        {
