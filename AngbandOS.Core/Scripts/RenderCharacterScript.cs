...
internal class RenderCharacterScript : Script, IScript, ICastSpellScript, IGameCommandScript
{
    ...
    private void DisplayPlayerAbilityScoresWithModifiers()
    {
        ...
        foreach (WieldSlot inventorySlot in Game.SingletonRepository.Get<WieldSlot>().Where(_inventorySlot => _inventorySlot.IsEquipment))
        {
            foreach (int i in inventorySlot.InventorySlots)
            {
                ...
                else
                {
                    ...
                    ShowBonus(itemCharacteristics.SustInt, itemCharacteristics.BonusIntelligence != 0, item.Characteristics.BonusIntelligence, row + 1, col);
        ...
        DisplayPlayerStatWithModification(Game.IntelligenceBonus, playerCharacteristics.BonusIntelligence != 0, row + 1, col);
