...
internal class LightsourceWieldSlot : EquipmentWieldSlot
{
    ...
    public override void ProcessWorld(ProcessWorldEventArgs processWorldEventArgs)
    {
        ...
        foreach (int index in InventorySlots)
        {
            ...
            if (oPtr != null && oPtr.EffectiveAttributeSet.Get<SummationEffectiveAttributeValue>(nameof(BurnRateAttribute)).Get() > 0 && oPtr.TurnsOfLightRemaining > 0)
            {
                hadLight = true;
                oPtr.TurnsOfLightRemaining -= oPtr.EffectiveAttributeSet.Get<SummationEffectiveAttributeValue>(nameof(BurnRateAttribute)).Get();
