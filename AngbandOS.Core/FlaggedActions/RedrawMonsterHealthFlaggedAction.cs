...
internal class RedrawMonsterHealthFlaggedAction : FlaggedAction
{
    ...
    {
        if (Game.TrackedMonsterIndex.Value == null)
        {
        ...
        else if (!Game.Monsters[Game.TrackedMonsterIndex.Value.Value].IsVisible)
        {
        ...
        else if (Game.Monsters[Game.TrackedMonsterIndex.Value.Value].Health < 0)
        {
            Game.Screen.Print(ColorEnum.White, "[----------]", RowInfo, ColInfo);
        }
        else
        {
            Monster mPtr = Game.Monsters[Game.TrackedMonsterIndex.Value.Value];
